...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DEFINICIÓN DE LA BASE DE DATOS MAESTRA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
define_database(pais_db/10, 
    [(nombre_pais, rfuzzy_string_type), 
     (anio, rfuzzy_integer_type), 
     (fsi_puntos, rfuzzy_integer_type), 
     (apoyo_puntos, rfuzzy_integer_type), 
     (temp_grados, rfuzzy_integer_type), 
     (camas_disp, rfuzzy_integer_type), 
     (pib_dolares, rfuzzy_integer_type),
     (capital_humano_x100, rfuzzy_integer_type), 
     (poblacion_urbana, rfuzzy_integer_type),    
     (lluvia_mm, rfuzzy_integer_type)]).        


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DATOS (HECHOS 2010-2019)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Orden: Nombre, Año, FSI, Apoyo(0-100), Temp, Camas, PIB, Capital Humano x100, % Urbano, Lluvia mm
...
% LIBIA (HC congelado en 200 como castigo analítico por estado fallido)
...
% FUNCIONES DIFUSAS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% -- 1. GEOPOLÍTICA (Índice FSI) --
estado_estable(pais_db) :~ function(fsi_puntos(pais_db), [ (0, 1), (40, 1), (65, 0) ]).
estado_precario(pais_db) :~ function(fsi_puntos(pais_db), [ (50, 0), (70, 1), (90, 0) ]).
alerta(pais_db) :~ function(fsi_puntos(pais_db), [ (80, 0), (105, 1), (130, 1) ]).
...
% -- 3. CLIMA (Temperatura Media Anual) --
...
% -- 4. SANIDAD (Camas por 1.000 hab - Escala x10) --
...
% -- 6. CAPITAL HUMANO (Índice x100) --
% Bajo (Menos de 200-240 es crítico, como Yemen o Siria en guerra).
capital_humano_bajo(pais_db) :~ function(capital_humano_x100(pais_db), [ (0, 1), (200, 1), (250, 0) ]).
...
% Una urbanización superior al 80% genera mucha presión si la economía falla (ej. Brasil, Argentina, Venezuela).
poblacion_urbana_alta(pais_db) :~ function(poblacion_urbana(pais_db), [ (60, 0), (80, 1), (100, 1) ]).

% -- 8. ESTRÉS HÍDRICO (Lluvia Total Anual en mm) --
% Por debajo de 300mm es un desierto extremo (Siria, Libia, Yemen).
...
% 1. Fractura de Estado (Política + Economía): 
riesgo_critico(pais_db) :~ rule(min, (alerta(pais_db), pobre(pais_db))).

% 2. La Trampa de la Ignorancia (Pobreza + Capital Humano Bajo):
riesgo_critico(pais_db) :~ rule(min, (pobre(pais_db), capital_humano_bajo(pais_db))).

% 3. El Polvorín Urbano (Ciudad Saturada + Estado Precario):
riesgo_critico(pais_db) :~ rule(min, (estado_precario(pais_db), poblacion_urbana_alta(pais_db))).

% 4. Estrés Hídrico y Hambruna (Sequía + Pobreza):
riesgo_critico(pais_db) :~ rule(min, (sequia_extrema(pais_db), pobre(pais_db))).

% 5. Colapso Sanitario Estructural (Sanidad + Aislamiento):
riesgo_critico(pais_db) :~ rule(min, (sanidad_precaria(pais_db), aislado(pais_db))).

% 6. Estado Fallido Puro (Alarma Absoluta): 
riesgo_critico(pais_db) :~ alerta(pais_db).

% VALOR BASE DE SEGURIDAD
riesgo_critico(pais_db) :~ defaults_to(0.1).